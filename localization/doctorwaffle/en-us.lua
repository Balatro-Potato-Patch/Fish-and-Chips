return {

    descriptions = {

        PotatoPatch = {

            PotatoPatchDev_waffle = {
                name = "waffle",
                text = {
                    "nondescript agender angler",
                    "{C:edition}My Nautical Roemance{} enjoyer",
                    "{s:0.8, C:inactive}also play wafflemod{}"
                }
            }

        },

        fac_Fish = {

            fish_fac_waffle_magic_conch = {
                name = "Magic Conch",
                text = {
                    "{C:attention}Once per round{}, ask the",
                    "{C:fac_fish}Magic Conch{} for a {C:attention}Food Joker{},",
                    "{C:green}#1# in #2#{} chance for it to say {C:attention}yes{}",
                    "{C:inactive}(Must have room)",
                    "{ppu_bubble:1}"
                },
                flavor = {
                    "Oh, Magic Conch Shell, what do we",
                    "need to do to defeat The Plant?"
                }
            },

            fish_fac_waffle_magic_conch_secret = {
                name = "Magic Conch",
                text = {
                    "{C:attention}Once per round{}, ask the",
                    "{C:fac_fish}Magic Conch{} for a {C:attention}Food Joker{},",
                    "{C:green}#1# in #2#{} chance for it to say {C:attention}yes{}",
                    "{C:inactive}(Must have room)",
                    "{ppu_bubble:1}"
                },
                flavor = {
                    "KILL EVERYONE",
                }
            },

            fish_fac_waffle_percheo = {
                name = "Percheo",
                text = {
                    "Creates a random {C:dark_edition}Negative{}",
                    "{C:tarot}Tarot{} or {C:planet}Planet{} card when",
                    "{C:attention}treasure{} is reeled in",
                },
                flavor = {
                    "Chat, is this nanefish?"
                }
            },

            fish_fac_waffle_dead_fish = {
                name = "Dead Fish",
                text = {
                    "Discarded {C:attention}2s{} earn",
                    "{C:fac_sand_dollars,f:fac_sand_dollars}$#1#{} and are {C:red}destroyed{}"
                },
                flavor = {
                    "2 x 2 = ?"
                }
            },

            fish_fac_waffle_squid_ink_cookie = {
                name = "Squid Ink Cookie",
                text = {
                    "When hand is played, lose {C:money}$#1#{}",
                    "and convert {C:attention}#2#{} random card#3#",
                    "in deck into {V:1}#4#{} suit"
                },
                flavor = {
                    "#1#"
                }
            },

            fish_fac_waffle_mudskipper = {
                name = "Mudskipper",
                text = {
                    "First {C:attention}catch{} each",
                    "fishing trip creates",
                    "a random {C:attention}Tag{}",
                    "{ppu_bubble:1}"
                },
                flavor = {
                    "Stop staring at me",
                    "with them big ol' eyes!"
                }
            },

            fish_fac_waffle_reginald = {
                name = "Reginald the Teleporting Sea Urchin",
                text = {
                    "Travel to a",
                    "different {C:fac_environment}Environment{}",
                    "after catching a {C:fac_fish}Fish{}"
                },
                flavor = {
                    "Currently on the run from the",
                    "nautical police due to multiple",
                    "accounts of undersea arson (somehow?)",
                }
            },

            fish_fac_waffle_double_dicefin = {
                name = "Double Dicefin",
                text = {
                    "{X:green,C:white}X#1#{} to all {C:attention}listed{} {C:green}probabilities{}",
                    "if a {C:attention}perfect catch{} was made",
                    "last fishing trip",
                    "{C:inactive}(ex: {C:green}1 in 3 {C:inactive}-> {C:green}#1# in 3{C:inactive})",
                    "{ppu_bubble:1}"
                },
                flavor = {
                    "This determined duo will roll with",
                    "each other until the bitter end."
                }
            },

            fish_fac_waffle_gossamer_worm = {
                name = "Gossamer Worm",
                text = {
                    "If catch is a {C:attention}perfect catch{} and",
                    "{C:attention}Treasure{} is reeled in, create a",
                    "random {C:spectral}Spectral{} card",
                    "{C:inactive}(Must have room)"
                },
                flavor = {
                    "Swimming through the twilight zone,",
                    "these glowing worms elegantly row their",
                    "paddle-like parapodia in search of food."
                }
            },

            fish_fac_waffle_bonus_duck = {
                name = "Bonus Duck",
                text = {
                    {
                        "When deck is shuffled, {C:attention}Ducks{}",
                        "appear on {C:attention}#1#%{} of cards in deck",
                        "{C:inactive}(rounded up)"
                    },
                    {
                        "This {C:fac_fish}Fish{} gains {C:chips}+#2#{} Chips whenever",
                        "cards with {C:attention}Ducks{} are scored",
                        "{C:inactive}(Currently {C:chips}+#3#{C:inactive} Chips)"
                    },
                },

                flavor = {
                    "Now the Ducks... are over there!",
                }
            },

            fish_fac_waffle_onion_fish = {
                name = "Onion Fish",
                text = {
                    {
                        "Lower the {C:attention}ranks{} of the",
                        "next {C:attention}#1#{} cards played",
                        "by {C:attention}1{} before scoring"
                    }
                },
                flavor = {
                    "Ogres are like onions. They stink."
                }
            },

            fish_fac_waffle_finclair = {
                name = "THE MAGNIFICENT FINCLAIR",
                text = {
                    "When {C:attention}Blind{} is selected, this {C:fac_fish}Fish{}",
                    "becomes a copy of {C:fac_fish}Fish{} to the {C:attention}right{}",
                    "with {X:attention,C:white}X#1#{} to {C:attention}listed values{}",
                    "{C:inactive}(Reverts at end of round, cannot",
                    "{C:inactive}copy THE MAGNIFICENT FINCLAIR)"
                },
                flavor = {
                    "Can we please pretend to like each other for 8 antes?",
                    "I'm going to be optimistic and say... sure.",
                },
            },

            fish_fac_waffle_worn_book = {
                name = "Worn Book",
                text = {
                    "Create {C:attention}#1#{} random {C:attention}Enhanced",
                    "{C:attention}#2#s{} and draw them to {C:attention}hand",
                    "{ppu_bubble:usable}"
                },
                flavor = {
                    "Fittingly enough, the book is",
                    "titled \"How to play Go Fish\"."
                }
            },

            fish_fac_waffle_worn_book_collection = {
                name = "Worn Book",
                text = {
                    "Create {C:attention}#1#{} random {C:attention}Enhanced",
                    "{C:attention}#2#s{} and draw them to {C:attention}hand",
                    "{C:inactive,s:0.85}(Chooses a random rank when caught)",
                    "{ppu_bubble:usable}"
                },
                flavor = {
                    "Fittingly enough, the book is",
                    "titled \"How to play Go Fish\"."
                }
            },

            fish_fac_waffle_handchovies = {
                name = "Handchovies",
                text = {
                    "If played hand contains {C:attention}#1#{}",
                    "{C:attention}scoring{} cards, gain {C:blue}+#2#{} Hand#3#"
                },
                flavor = {
                    "These saltwater fish make for",
                    "quite the savory topping, but",
                    "are an acquired taste for sure."
                }
            },

            fish_fac_waffle_unemployster = {
                name = "Unemployster",
                text = {
                    "Instead of earning {C:fac_sand_dollars}Sand{}",
                    "{C:fac_sand_dollars}Dollars{}, this {C:fac_fish}Fish{} gains",
                    "{C:white,X:mult}X#1#{} Mult per {C:fac_sand_dollars,f:fac_sand_dollars}$1{} earned",
                    "{C:inactive}(Currently {C:white,X:mult}X#2#{C:inactive} Mult)"
                },
                flavor = {
                    "It's hard to get a job when you're",
                    "literally a sessile organism."
                }
            },

            fish_fac_waffle_scaly_foot_snail = {
                name = "Scaly-Foot Snail",
                text = {
                    "When entering a {C:attention}Boss Blind{},",
                    "all cards in {C:attention}first hand drawn{}",
                    "are enhanced into {C:attention}Steel Cards{},",
                    "then {C:red,E:2}self destructs{}"
                },
                flavor = {
                    "Also known as the volcano snail,",
                    "these creatures live in hydrothermal",
                    "vents that reach temperatures of 400°C"
                }
            },

            fish_fac_waffle_pyukumuku = {
                name = "Pyukumuku",
                text = {
                    {
                        "This {C:fac_fish}Fish{} stores {C:chips}+#1#{} Chips",
                        "when a played card is {C:attention}scored{}",
                        "{C:inactive}(Currently {C:chips}#2#{C:inactive} Chips)"
                    },
                    {
                        "If played hand is {C:attention}exactly{}",
                        "{C:attention}1{} card, permanently add",
                        "this {C:fac_fish}Fish's{} stored {C:chips}Chips{}",
                        "to it and {C:red}reset{} stored {C:chips}Chips{}"
                    }
                },
                flavor = {
                    "Pyukumuku can eject its internal organs,",
                    "using them to attack prey and enemies alike."
                }
            },

            fish_fac_waffle_self_finsert = {
                name = "Self-Finsert",
                text = {
                    "Becomes a random {C:fac_fish}Fish",
                    "made by {V:1}waffle{} with",
                    "a random {C:dark_edition}Edition",
                    "{C:inactive}(Self-Finsert excluded,",
                    "{C:inactive}{C:dark_edition}Negative{C:inactive} excluded)",
                    "{ppu_bubble:usable}"
                },
                flavor = {
                    "The undisputed NEET",
                    "of the seven seas."
                }
            }

        },

        Other = {
            fac_waffle_finclair_tooltip = {
                name = "Finclair Copy",
                text = {
                    "This {C:fac_fish}Fish{} will turn into",
                    "{C:attention}THE MAGNIFICENT FINCLAIR{}",
                    "at end of round, or",
                    "if used and consumed",
                }
            }
        },

    },

    misc = {

        dictionary = {
            k_fac_waffle_ready_ex = "Ready!",
            k_fac_waffle_tag = "+1 Tag",
            k_fac_waffle_inactive = "inactive",
            k_fac_waffle_cookie = "Cookie",
            k_fac_waffle_conch = "Plastic Toy",
            k_fac_waffle_echinoderm = "Echinoderm",
            k_fac_waffle_invertebrate = "Invertebrate",
            k_fac_waffle_duck = "Duck",
            k_fac_waffle_presto_ex = "Presto!",
            k_fac_waffle_book = "Book",
            k_fac_waffle_plus_hand = "+1 Hand",
            k_fac_waffle_mollusc = "Mollusc",
            k_fac_waffle_gastropod = "Gastropod",
            k_fac_waffle_steel_ex = "Steel!",
            k_fac_waffle_pokemon = "Pokémon",

            fac_waffle_squid_ink1 = "Hungry...",
            fac_waffle_squid_ink2 = "Sorrow...",
            fac_waffle_squid_ink3 = "Sob... sob...",
            fac_waffle_squid_ink4 = "Don't know...",
            fac_waffle_squid_ink5 = "Who... am...",
            fac_waffle_squid_ink6 = "Sob sob...",
            fac_waffle_squid_ink7 = "Shiny... things...",
            fac_waffle_squid_ink8 = "Dark... lonely...",
            fac_waffle_squid_ink9 = "Lost... everything...",
            fac_waffle_squid_ink10 = "Haaah...",
            fac_waffle_squid_ink11 = "Into... water...",
            fac_waffle_squid_ink12 = "Remember... nothing..."
        },

    }

}