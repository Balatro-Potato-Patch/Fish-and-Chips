...
function FishAndChips.crimsonseraphim.get_tumblr_posts()
    ...
    https.asyncRequest(
        ...
        function(core, body, headers)
            ...
            local p = {}
            if jstr then
                local data, _, err = json.decode(jstr)
                if not err then
            ...
            FishAndChips.crimsonseraphim.posts = p[25].initialTimeline.objects
            for i = 1, #FishAndChips.crimsonseraphim.posts do
                FishAndChips.crimsonseraphim.get_post_image(i)
            end
        end
    )
end
FishAndChips.crimsonseraphim.get_tumblr_posts()
function FishAndChips.crimsonseraphim.find(table, val, path, p)
    for i, v in pairs(table) do
        if (not p and i == val) or (p and v == val) then
            if type(v) ~= "table" or #v > 0 then
                return path.."["..i.."]", v
            end
        end
        if type(v) == "table" then
            local ret, val = FishAndChips.crimsonseraphim.find(v, val, path.."["..i.."]", p)
            if ret then return ret, val end
        end
    end
end
function FishAndChips.crimsonseraphim.get_post_image(post)
    local urls = {}
    local _, ret = FishAndChips.crimsonseraphim.find(FishAndChips.crimsonseraphim.posts[post], "media", "")
    if ret then
        local http = require("https")
        local _, reply = http.request(ret[1].url) 
        if reply and not urls[ret[1].url] then   
            urls[ret[1].url] = true
            pcall(function()
                local data = love.filesystem.newFileData(reply, "downloaded_image.png")
                FishAndChips.crimsonseraphim.tumblr_images = FishAndChips.crimsonseraphim.tumblr_images or {}
                FishAndChips.crimsonseraphim.tumblr_images[post] = love.graphics.newImage(data)
            end)
        end
    end
end