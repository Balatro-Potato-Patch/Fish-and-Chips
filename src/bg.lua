...
FishAndChips.bg_manager = manager
local conf = FishAndChips.mod.config
local cooldown = 1.1
local target = 0.016
local lw, lh
...
function manager:draw()
    ...
    local shader = G.SHADERS.fac_fas_water
    shader:send("water_height", -100)
    shader:send("fas_water", {
        G.TIMERS.REAL/28,
        G.TIMERS.REAL
    })
    shader:send("uie_details", {1500,500,200,270}) -- We love magic numbers
